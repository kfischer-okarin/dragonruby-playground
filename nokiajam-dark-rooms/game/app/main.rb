...
def wall_render_options(room, wall)
  {}.tap { |result|
    result.update ROTATED_BY_90_DEGREES if %i[left right].include?(wall)
    result[:invert] = true if room[:light]
  }
end

ROTATED_BY_90_DEGREES = { angle: 90, angle_anchor_x: 0, angle_anchor_y: 0.5 }.freeze
...
def render_room(room)
  ...
  $outputs.render_sprite left, bottom + Sprites::WALL_LENGTH - 1, wall_type(room, :up), wall_render_options(room, :up)
  $outputs.render_sprite left, bottom - 1, wall_type(room, :left), wall_render_options(room, :left)
  $outputs.render_sprite left + Sprites::WALL_LENGTH - 1, bottom - 1, wall_type(room, :right), wall_render_options(room, :right)
  $outputs.render_rect left + 1, bottom + 1, Sprites::WALL_LENGTH - 2, Sprites::WALL_LENGTH - 2 if room[:light]
end
